...
RetreiveTiltMotorFbkMsgs::RetreiveTiltMotorFbkMsgs()
{

}

RetreiveTiltMotorFbkMsgs::RetreiveTiltMotorFbkMsgs(const std::string sender_id_name)
{
	sptr_middleware_ = std::make_shared<Platform::Sensors::SensorsStkciSubData>();
	std::vector<std::string> sub_interface_names;
	sub_interface_names.push_back("TiltMotorFbkMsg");
...
void RetreiveTiltMotorFbkMsgs::operator()()
{
	Platform::Sensors::TiltMotorFbkMsg out_datas;

	//Retrieve data from stkci
	//if(sptr_middleware_->SubPalletBaseCmd(sender_id_name_,out_datas,200))
	if(sptr_middleware_->SubTiltMotorFbkMsg(sender_id_name_,out_datas))
	{
...
bool RetreiveTiltMotorFbkMsgs::GetTiltMotorFbkMsgs(Platform::Sensors::TiltMotorFbkMsg &out_data)
{
	bool success=false;
...
bool RetreiveTiltMotorFbkMsgs::GetLiveliness()
{
