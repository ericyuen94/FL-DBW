...
RetreivePIMCmd::RetreivePIMCmd()
{

}

RetreivePIMCmd::RetreivePIMCmd(const std::string sender_id_name)
{
	sptr_middleware_ = std::make_shared<Platform::Sensors::SensorsStkciSubData>();
	std::vector<std::string> sub_interface_names;
	sub_interface_names.push_back("PalletAuxCmd");
...
void RetreivePIMCmd::operator()()
{
	Platform::Sensors::PalletAuxCmdMsg out_datas;

	//Retrieve data from stkci
	if(sptr_middleware_->SubPalletAuxCmd(sender_id_name_,out_datas))
	{
		std::unique_lock<std::mutex> lock(mutex_data_PIM_data);
...
bool RetreivePIMCmd::GetPIMDataMsg(Platform::Sensors::PalletAuxCmdMsg &out_data)
{
	bool success=false;
...
bool RetreivePIMCmd::GetLiveliness()
{
