...
RetreiveTiltMotorCmds::RetreiveTiltMotorCmds()
{

}

RetreiveTiltMotorCmds::RetreiveTiltMotorCmds(const std::string sender_id_name)
{
	sptr_middleware_ = std::make_shared<Platform::Sensors::SensorsStkciSubData>();
	std::vector<std::string> sub_interface_names;
	sub_interface_names.push_back("TiltMotorMsg");
...
void RetreiveTiltMotorCmds::operator()()
{
	Platform::Sensors::TiltMotorMsg out_datas;

	//Retrieve data from stkci
	//if(sptr_middleware_->SubPalletBaseCmd(sender_id_name_,out_datas,200))
	if(sptr_middleware_->SubTiltMotorMsg(sender_id_name_,out_datas))
	{
...
bool RetreiveTiltMotorCmds::GetTiltMotorMsgs(Platform::Sensors::TiltMotorMsg &out_data)
{
	bool success=false;
...
bool RetreiveTiltMotorCmds::GetLiveliness()
{
