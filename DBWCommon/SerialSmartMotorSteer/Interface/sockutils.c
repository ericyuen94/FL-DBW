...
int mu_sock_write_string(int sock, char *s)
{
  ...
  n = mu_sock_writen(sock, s, length, -1);      
  if (n < 0)
    return n;

  c = '\r';
  return_val = mu_sock_writen(sock, &c, 1, -1);
  if (return_val < 0)
    return return_val;  
...
int mu_sock_printf(int sock, char *fmt, ...) 
{
  ...
  va_end(args);
  if (n > -1 && n < 4096) {    
    return mu_sock_write_string(sock, Buffer);
  }
  return -1;
}
...
int mu_sock_clear_input_buffer(int sock)
{
  ...
  val = mu_sock_bytes_available(sock);
  if(val > 0) {
